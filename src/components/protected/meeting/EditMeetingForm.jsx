...
import useEditMeetingMutation from "@/hooks/mutations/useEditMeetingMutation";

export default function EditMeetingForm({ meetingDetail, meetingId }) {
  const { data } = useRoom();
  const mutation = useEditMeetingMutation(meetingId);

  // 1. Define your form.
  const form = useForm({
    resolver: zodResolver(meetingSchema),
    defaultValues: {
      title: meetingDetail?.data.title,
      content: meetingDetail?.data.content,
      day: new Date(meetingDetail?.data.day),
      start_time: meetingDetail?.data.start_time,
      end_time: meetingDetail?.data.end_time,
      extra_notes: meetingDetail?.data.extra_notes,
      room: meetingDetail?.data.room || 0,
    },
  });
