...
export default function ScheduleMeetings() {
  ...
  const transformedData = data?.data?.map((item) => {
    // Parse the date-time as a UTC date
    const startTime = new Date(`${item.day} ${item.start_time}.919320`);
    const endTime = new Date(`${item.day} ${item.end_time}.919320`);
  
    return {
      Id: item.id,
      Subject: item.title,
      StartTime: startTime.toISOString(), // Ensure UTC format
      EndTime: endTime.toISOString(),     // Ensure UTC format
      Description: item.content,
      Notes: item.extra_notes || 'N/A',
    ...
    if (props) {
      return (
        <div className="text-[16px] flex flex-col gap-2">
          ...
          <p>
            <b className="block">Start Time:</b>{" "}
            {new Date(props.StartTime).toLocaleString()}
          </p>
          <p>
            <b className="block">End Time:</b>{" "}
